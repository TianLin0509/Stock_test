...
from ui.results import run_analysis, show_completed_results
...
""", unsafe_allow_html=True)
    ...
    with st.sidebar:
        ...
        selected_model = st.selectbox(
            "当前模型", options=MODEL_NAMES, index=0,
...
""", unsafe_allow_html=True)
  ...
  K线图和财务数据将无法显示，请检查网络连接或联系管理员检查 Token。
    ...
    with bc1:
        btn_exp = st.button(
            "✅ 预期差已完成" if analyses.get("expectation") else "🔍 预期差分析",
            use_container_width=True,
        )
    with bc2:
        btn_trend = st.button(
            "✅ 趋势已完成" if analyses.get("trend") else "📈 K线趋势",
            use_container_width=True,
        )
    with bc3:
        btn_fund = st.button(
            "✅ 基本面已完成" if analyses.get("fundamentals") else "📋 基本面",
            use_container_width=True,
        )
    with bc4:
        btn_moe = st.button(
  ...
  ⚠️ <strong>部分数据获取受限</strong>：{' | '.join(data_errors[:3])}
    ...
    # 检测哪些按钮被点了
    any_analysis_btn = btn_exp or btn_trend or btn_fund or btn_moe or btn_qa or btn_all

    if any_analysis_btn and not stock_ready:
  ...
  ⚠️ <strong>请先点击「🔍 查询股票」获取数据</strong>，然后再进行分析。
        ...
        if ai_err and any_analysis_btn:
            st.markdown(f"""<div class="status-banner warn">
  ...
  请在左侧切换其他模型。
        ...
        # ── 分发到 results 模块执行 ───────────────────────────────────
        actions = {
            "expectation": btn_exp or btn_all,
            "trend":       btn_trend or btn_all,
            "fundamentals": btn_fund or btn_all,
            "moe":         btn_moe or btn_all,
            "qa":          btn_qa,
        }
        run_analysis(client, cfg_now, selected_model, actions)
